...
#    `functionName` is a function's name in your module (e.g., `mpbWindEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/achubaty/Documents/GitHub/MPB/SpaDES/modules/mpbWind/tests/testthat`).
...
test_that("test Event1 and Event2.", {
  ...
  parameters <- list(
    ...
    mpbWind = list(.saveInitialTime = NA)
  )
  ...
  if (exists("mpbWindEvent1", envir = .GlobalEnv)) {
    simOutput <- mpbWindEvent1(mySim)
  } else {
    simOutput <- mySim$mpbWindEvent1(mySim)
  }
  ...
  if (exists("mpbWindEvent2", envir = .GlobalEnv)) {
    simOutput <- mpbWindEvent2(mySim)
  } else {
    simOutput <- mySim$mpbWindEvent2(mySim)
  }
